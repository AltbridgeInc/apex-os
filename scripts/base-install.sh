...
# Installs APEX-OS from GitHub repository to ~/apex-os
...
spinner() {
    ...
    while true; do
        for dot_count in "" "." ".." "..."; do
            echo -ne "\r${BLUE}Installing APEX-OS files${dot_count}${NC}   "
            sleep $delay
        done
    done
}
...
install_all_files() {
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        if [[ "$VERBOSE" != "true" ]]; then
            ...
            print_status "Installing APEX-OS files..."
        fi
    fi
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        if [[ $download_status -eq 0 && $file_count -gt 0 ]]; then
            echo "✓ Installed $file_count files to ~/apex-os"
        else
...
prompt_overwrite_choice() {
    ...
    echo "You already have a base installation of APEX-OS"
    ...
    echo "    - ~/apex-os/profiles/default/*"
    echo "    - ~/apex-os/scripts/*"
    echo "    - ~/apex-os/CHANGELOG.md"
    echo ""
    echo "    Updates your version number in ~/apex-os/config.yml but doesn't change anything else in this file."
    echo ""
    echo "    Everything else in your ~/apex-os folder will remain intact."
    echo ""
    ...
    echo ""
    echo "    - Makes a backup of your current ~/apex-os folder at ~/apex-os.backup"
    echo "    - Deletes your current ~/apex-os folder and all of its contents."
    echo "    - Installs a fresh ~/apex-os base installation"
    echo ""
...
create_backup() {
    ...
    echo "✓ Backed up existing installation to ~/apex-os.backup"
    echo ""
}
...
perform_fresh_installation() {
    ...
    echo -e "  Target: ${YELLOW}~/apex-os${NC}"
    echo ""

    # Create base directory
    ensure_dir "$BASE_DIR"
    echo "✓ Created base directory: ~/apex-os"
    echo ""
    ...
    echo ""
    print_success "APEX-OS has been successfully installed!"
    echo ""
    echo -e "${GREEN}Next steps:${NC}"
    echo ""
    echo -e "${GREEN}1) Customize your profile's standards in ~/apex-os/profiles/default/principles${NC}"
    echo ""
    echo -e "${GREEN}2) Navigate to a project directory${NC}"
    echo -e "   ${YELLOW}cd path/to/project-directory${NC}"
    echo ""
    echo -e "${GREEN}3) Install APEX-OS in your project by running:${NC}"
    echo -e "   ${YELLOW}~/apex-os/scripts/project-install.sh${NC}"
    echo ""
    echo -e "${GREEN}Visit the docs for guides on how to use APEX-OS: https://github.com/AltbridgeInc/apex-os${NC}"
    echo ""
}
...
main() {
    print_section "APEX-OS Base Installation"
