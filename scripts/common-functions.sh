...
# =============================================================================
# APEX-OS Common Functions
# Shared utilities for APEX-OS scripts
# =============================================================================
...
process_workflows() {
    ...
    while IFS= read -r workflow_ref; do
        ...
        if [[ -f "$workflow_file" ]]; then
            ...
            local warning_msg="⚠️ This workflow file was not found in your APEX-OS base installation at ~/apex-os/profiles/$profile/workflows/${workflow_path}.md"
...
process_phase_tags() {
    ...
    # Find all PHASE tags: {{PHASE X: @apex-os/commands/path/to/file.md}}
    ...
    while IFS= read -r phase_ref; do
        ...
        if [[ "$mode" == "embed" ]]; then
            ...
            # Extract: {{PHASE 1: @apex-os/commands/plan-product/1-product-concept.md}}
            # To get: PHASE 1, plan-product/1-product-concept.md, "Product Concept"

            local phase_label=$(echo "$phase_ref" | sed 's/{{//' | sed 's/:.*$//')  # "PHASE 1"
            local file_ref=$(echo "$phase_ref" | sed 's/.*@apex-os\/commands\///' | sed 's/}}$//')  # "plan-product/1-product-concept.md"
...
# Check if APEX-OS is installed in project
...
get_project_config() {
    ...
    get_yaml_value "$project_dir/apex-os/config.yml" "$key" ""
}
...
check_not_base_installation() {
    if [[ -f "$PROJECT_DIR/apex-os/config.yml" ]]; then
        if grep -q "base_install: true" "$PROJECT_DIR/apex-os/config.yml"; then
            echo ""
            print_error "Cannot install APEX-OS in base installation directory"
            echo ""
            echo "It appears you are in the location of your APEX-OS base installation (your home directory)."
            echo "To install APEX-OS in a project, move to your project's root folder:"
            echo ""
...
write_project_config() {
    ...
    local dest="$PROJECT_DIR/apex-os/config.yml"
...
#
# To change these settings, run ~/apex-os/scripts/project-update.sh to re-compile your project with the new settings.
# ================================================
profile: $profile
...
create_standard_skill() {
    ...
    # Create skill directory (directly in .claude/skills/, not in apex-os subfolder)
    ...
    # Prepend apex-os/ to the standards file path for the file reference
    local standard_file_path_with_prefix="apex-os/$standards_file"
...
install_improve_skills_command() {
    ...
    local target_dir="$PROJECT_DIR/.claude/commands/apex-os"
    mkdir -p "$target_dir"
