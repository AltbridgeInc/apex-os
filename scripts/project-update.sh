...
# =============================================================================
# APEX-OS Project Update Script
# Updates APEX-OS installation in a project
# =============================================================================
...
Update APEX-OS installation in the current project directory.
...
Options:
    ...
    --apex-os-commands [BOOL]               Install apex-os commands for other tools (true/false)
    --standards-as-claude-code-skills [BOOL] Use Claude Code Skills for standards (true/false)
    --re-install                             Delete and reinstall APEX-OS
...
validate_installations() {
    ...
    if [[ ! -f "$PROJECT_DIR/apex-os/config.yml" ]]; then
        print_error "APEX-OS not installed in this project"
        echo ""
        print_status "Please run project-install.sh first"
        exit 1
    fi

    print_verbose "Project installation found at: $PROJECT_DIR/apex-os"
}
...
load_configurations() {
    ...
    print_verbose "  APEX-OS commands: $BASE_AGENT_OS_COMMANDS"
    ...
    print_verbose "  APEX-OS commands: $PROJECT_AGENT_OS_COMMANDS"
    ...
    print_verbose "  APEX-OS commands: $EFFECTIVE_AGENT_OS_COMMANDS"
...
update_standards() {
    ...
    while read file; do
        if [[ "$file" == standards/* ]]; then
            ...
            local dest="$PROJECT_DIR/apex-os/$file"
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        if [[ $standards_new -gt 0 ]]; then
            echo "✓ Added $standards_new standards in apex-os/standards"
        fi
        if [[ $standards_updated -gt 0 ]]; then
            echo "✓ Updated $standards_updated standards in apex-os/standards"
        fi
        if [[ $standards_skipped -gt 0 ]]; then
            echo -e "${YELLOW}$standards_skipped files in apex-os/standards were not updated and overwritten. To update and overwrite these, re-run with --overwrite-standards flag.${NC}"
        fi
    fi
}
        ...
        if [[ "$file" == commands/*/single-agent/* ]] || [[ "$file" == commands/orchestrate-tasks/orchestrate-tasks.md ]]; then
            ...
            if [[ -f "$source" ]]; then
                ...
                if [[ "$file" == commands/orchestrate-tasks/orchestrate-tasks.md ]]; then
                    local dest="$PROJECT_DIR/apex-os/commands/orchestrate-tasks/orchestrate-tasks.md"
                else
                    # Strip the single-agent/ subfolder for apex-os/commands structure
                    local dest_file=$(echo "$file" | sed 's/\/single-agent//')
                    local dest="$PROJECT_DIR/apex-os/$dest_file"
                fi
...
update_claude_code_files() {
    ...
    # Update commands in .claude/commands/apex-os/
        ...
        while read file; do
            if [[ "$file" == commands/*/multi-agent/* ]] || [[ "$file" == commands/orchestrate-tasks/orchestrate-tasks.md ]]; then
                ...
                if [[ -f "$source" ]]; then
                    ...
                    local dest="$PROJECT_DIR/.claude/commands/apex-os/${command_name}.md"
        ...
        while read file; do
            if [[ "$file" == commands/*/single-agent/* ]] || [[ "$file" == commands/orchestrate-tasks/orchestrate-tasks.md ]]; then
                ...
                if [[ -f "$source" ]]; then
                    ...
                    if [[ "$file" == commands/orchestrate-tasks/orchestrate-tasks.md ]]; then
                        local dest="$PROJECT_DIR/.claude/commands/apex-os/orchestrate-tasks.md"
                        ...
                        if [[ ! "$filename" =~ ^[0-9]+-.*\.md$ ]]; then
                            ...
                            local dest="$PROJECT_DIR/.claude/commands/apex-os/$cmd_name.md"
    ...
    get_profile_files "$PROJECT_PROFILE" "$BASE_DIR" "agents" | while read file; do
        if [[ "$file" == agents/*.md ]] && [[ "$file" != agents/templates/* ]]; then
            ...
            if [[ -f "$source" ]]; then
                ...
                local dest="$PROJECT_DIR/.claude/agents/apex-os/${agent_name}.md"
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        local command_pattern=".claude/commands/apex-os"
        ...
        local agent_pattern=".claude/agents/apex-os"
        local agents_updated=0
        local agents_skipped=0
        local agents_new=0
...
# Update apex-os folder and configuration
update_agent_os_folder() {
    print_status "Updating apex-os folder"
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        echo "✓ Updated apex-os folder"
        echo "✓ Updated apex-os project configuration"
    fi
}
...
perform_update() {
    ...
    echo -e "  APEX-OS commands: ${YELLOW}$PROJECT_AGENT_OS_COMMANDS${NC}"
    echo ""
    ...
    # Update apex-os commands if enabled
...
prompt_update_confirmation() {
    ...
    if [[ "$has_version_diff" == "true" ]] || [[ "$has_config_diff" == "true" ]]; then
        ...
        echo ""
        print_status "Your project's APEX-OS version and/or configuration is different than the version you're trying to install."
    else
        echo ""
    ...
    print_status "Current project's APEX-OS:"
    ...
    if [[ -n "${MULTI_AGENT_MODE:-}" ]] || [[ -n "${SINGLE_AGENT_MODE:-}" ]] || [[ -n "${MULTI_AGENT_TOOL:-}" ]]; then
        ...
        echo "  APEX-OS commands: ${PROJECT_AGENT_OS_COMMANDS:-false}"
    ...
    echo "  APEX-OS commands: $EFFECTIVE_AGENT_OS_COMMANDS"
    ...
    echo ""
    echo "  - apex-os/config.yml"
    echo "  - apex-os/standards/"
    if [[ "$EFFECTIVE_AGENT_OS_COMMANDS" == "true" ]] || [[ -d "$PROJECT_DIR/apex-os/commands" ]]; then
        echo "  - apex-os/commands/"
    fi
    if [[ "$EFFECTIVE_USE_CLAUDE_CODE_SUBAGENTS" == "true" ]] || [[ -d "$PROJECT_DIR/.claude/agents/apex-os" ]]; then
        echo "  - .claude/agents/apex-os/"
    fi
    if [[ "$EFFECTIVE_CLAUDE_CODE_COMMANDS" == "true" ]] || [[ -d "$PROJECT_DIR/.claude/commands/apex-os" ]]; then
        echo "  - .claude/commands/apex-os/"
    fi
    if [[ "$EFFECTIVE_STANDARDS_AS_CLAUDE_CODE_SKILLS" == "true" ]] || [[ -d "$PROJECT_DIR/.claude/skills" ]]; then
        echo "  - .claude/skills/ (APEX-OS skills)"
    fi
    echo ""
...
perform_update_cleanup() {
    ...
    # Delete apex-os/standards/ (will be reinstalled)
    if [[ -d "$PROJECT_DIR/apex-os/standards" ]]; then
        print_status "Removing apex-os/standards/"
        if [[ "$DRY_RUN" != "true" ]]; then
            rm -rf "$PROJECT_DIR/apex-os/standards"
        fi
    fi

    # Delete apex-os/commands/ if exists
    if [[ -d "$PROJECT_DIR/apex-os/commands" ]]; then
        print_status "Removing apex-os/commands/"
        if [[ "$DRY_RUN" != "true" ]]; then
            rm -rf "$PROJECT_DIR/apex-os/commands"
        fi
    fi

    # Delete .claude/agents/apex-os/ if exists
    if [[ -d "$PROJECT_DIR/.claude/agents/apex-os" ]]; then
        print_status "Removing .claude/agents/apex-os/"
        if [[ "$DRY_RUN" != "true" ]]; then
            rm -rf "$PROJECT_DIR/.claude/agents/apex-os"
        fi
    fi

    # Delete .claude/commands/apex-os/ if exists
    if [[ -d "$PROJECT_DIR/.claude/commands/apex-os" ]]; then
        print_status "Removing .claude/commands/apex-os/"
        if [[ "$DRY_RUN" != "true" ]]; then
            rm -rf "$PROJECT_DIR/.claude/commands/apex-os"
        fi
    fi

    # Delete old .claude/skills/apex-os/ if exists (legacy location)
    if [[ -d "$PROJECT_DIR/.claude/skills/apex-os" ]]; then
        print_status "Removing legacy .claude/skills/apex-os/"
        if [[ "$DRY_RUN" != "true" ]]; then
            rm -rf "$PROJECT_DIR/.claude/skills/apex-os"
        fi
    fi

    # Delete individual APEX-OS skills (new location: .claude/skills/[skill-name]/)
    ...
    # Delete apex-os/roles/ if exists (legacy)
    if [[ -d "$PROJECT_DIR/apex-os/roles" ]]; then
        print_status "Removing legacy apex-os/roles/"
        if [[ "$DRY_RUN" != "true" ]]; then
            rm -rf "$PROJECT_DIR/apex-os/roles"
        fi
    fi
