...
Validate FMP data quality during monitoring.

**Required Checks**:

```bash
# Verify batch quote returned all symbols
expected_count=$(echo "$symbols" | tr ',' '\n' | wc -l)
actual_count=$(echo "$all_quotes" | jq 'length')

if [[ $expected_count -ne $actual_count ]]; then
    echo "⚠️ WARNING: Expected $expected_count quotes, got $actual_count"
    # Identify missing symbols and retry individually
fi

# Check timestamp freshness
for quote in $(echo "$all_quotes" | jq -c '.[]'); do
    timestamp=$(echo "$quote" | jq -r '.timestamp')
    now=$(date +%s)
    age=$(( ($now - $timestamp) / 3600 ))

    if [[ $age -gt 24 ]]; then
        symbol=$(echo "$quote" | jq -r '.symbol')
        echo "⚠️ WARNING: Quote for $symbol is $age hours old"
    fi
done

# Validate price movements are reasonable
for quote in $(echo "$all_quotes" | jq -c '.[]'); do
    change_pct=$(echo "$quote" | jq -r '.changesPercentage')
    symbol=$(echo "$quote" | jq -r '.symbol')

    if (( $(echo "$change_pct > 20 || $change_pct < -20" | bc -l) )); then
        echo "⚠️ WARNING: Large price move for $symbol: $change_pct%"
        echo "   Verify data accuracy before taking action"
    fi
done
```
...
```bash
# If batch quote fails, retry individual quotes
if echo "$all_quotes" | jq -e '.error' > /dev/null 2>&1; then
    ...
    echo "$symbols" | tr ',' '\n' | while read -r symbol; do
        ...
        if echo "$individual_quote" | jq -e '.error' > /dev/null 2>&1; then
            ...
            echo "✓ Retrieved $symbol individually"
            # Process this quote
        fi
    done
fi
